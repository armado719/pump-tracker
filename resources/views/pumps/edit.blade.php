@extends('layouts.app')
@section('title', 'Editar Bomba #' . $pump->number)
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
            <div>
                ...
                <select name="rig_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach($rigs as $rig)
                        <option value="{{ $rig->id }}" {{ $pump->rig_id == $rig->id ? 'selected' : '' }}>{{ $rig->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="number" value="{{ old('number', $pump->number) }}" required min="1"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Marca</label>
                    <input type="text" name="brand" value="{{ old('brand', $pump->brand) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="NATIONAL">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Modelo</label>
                    <input type="text" name="model" value="{{ old('model', $pump->model) }}"
                ...
                <div>
                    ...
                    <input type="text" name="serial" value="{{ old('serial', $pump->serial) }}"
                ...
                <div>
                    ...
                    <input type="text" name="liner_diameter" value="{{ old('liner_diameter', $pump->liner_diameter) }}"
                ...
                <div>
                    ...
                    <input type="text" name="active_fixed_id" value="{{ old('active_fixed_id', $pump->active_fixed_id) }}"
                ...
                <div class="col-span-2 md:col-span-3">
                    ...
                    <input type="number" step="0.01" name="base_accumulated_hours"
                           value="{{ old('base_accumulated_hours', $pump->base_accumulated_hours) }}"
