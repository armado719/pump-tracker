...
<div class="pt-4 max-w-md mx-auto space-y-5">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="font-semibold text-gray-800 mb-5 text-base">Crear nuevo pozo</h2>

        <form method="POST" action="{{ route('wells.store') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <select name="rig_id" required
                    ...
                    <option value="">— Selecciona un rig —</option>
                    @foreach($rigs as $rig)
                        <option value="{{ $rig->id }}" {{ old('rig_id') == $rig->id ? 'selected' : '' }}>
                            {{ $rig->name }}
                        </option>
                    @endforeach
                </select>
